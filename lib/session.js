...
async function updateUserSession(sessionId, newStuff = {}) {
	if (!isValidSessionId(sessionId)) return {}
	if (!client) initClient()
	try {
		const stuffInRedis = getUserSession(sessionId)
		const newObj = Object.assign({}, stuffInRedis, newStuff)
		await client.set(sessionId, JSON.stringify(newObj))
		return sessionId
	} catch(error) {
		log(error)
	}
