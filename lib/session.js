...
/**
 * Updates session entry in Redis. Takes the existing object, appends/
 * overwrites it with the passed new object, then writes it back to Redis.
 * Returns the session id.
 *
 * Note: the access token stored in Redis with createUserSession is not
 * protected, so it can be overwritten with this function
 *
 * @param  {String} sessionId
 * @param  {Object} data
 * @return {Number}
 */
async function updateUserSession(sessionId, data = {}) {
	if (!isValidSessionId(sessionId)) return {}
	if (!client) initClient()
	try {
		const sessionData = getUserSession(sessionId)
		await client.set(sessionId, JSON.stringify({ ...sessionData, data }))
		return sessionId
	} catch(error) {
		log(error)
	}
